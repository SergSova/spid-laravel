...
function init() {
    ...
    function resizeElents() {
        ...
        for (var i = 0; i < resizeEl.length; i++) {
          ...
          if(!resizeEl[i].classList.contains('spin-image')){
            ...
            if (transOrig) {
                resizeEl[i].style.transformOrigin = '50% ' + '50% ' + transOrig * uniKoef + 'px';
            }
        }
  ...
  // $('h1').html($('.rotator-row-wrap').width())
    /* Rotator Game */
    ...
    function Spiner(el, uniq) {
        ...
        this.spinStart = function (n, m, z) {
            _this = this.el
            _thisChildren = _this.children
            _thisLast = _this.children[_this.children.length - 1]
            _thisActive = _this.children[2];
            ...
            z--
            for (var i = 0; i < _thisChildren.length; i++) {
                _thisChildren[i].style.transition = 'all ' + (time ) + 'ms linear';
            }
            setTimeout(function () {
            ...
            _this.prepend(_thisLast);
        }

        this.spinSpin = function (n, m, z) {
          _this = this.el
          _thisChildren = _this.children;
          _thisLast = _this.children[_this.children.length - 1];
          _thisActive = _this.children[4];
          time = 130;
        ...
        this.spinStop = function (n, m, z) {
          _this = this.el;
          _thisChildren = _this.children;
          _thisLast = _this.children[_this.children.length - 1];
          _thisActive = _this.children[3];
          _thisbefore = _this.children[2];


          if (!_thisbefore.classList.contains('spin' + m)) {
            time = (130);
            time1 = 'all ' + 130 + 'ms linear'
          } else {
          ...
          z++
          for (var i = 0; i < _thisChildren.length; i++) {
            _thisChildren[i].style.transition = time1;
          }
          setTimeout(function () {
