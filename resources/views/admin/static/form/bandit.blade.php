    ...
    @foreach(\App\Http\Middleware\Locale::$languages as $lang)
        <div class="tab-pane fade {{$loop->first?'show active':''}}" id="nav-{{$lang}}" role="tabpanel"
             aria-labelledby="nav-{{$lang}}-tab">
                ...
                <div class=" form-group">
                    {{ Form::label('right_'.$lang,'Правильный вариант') }}
                    {{ Form::text('right_'.$lang,NULL, ['class'=>'form-control']) }}
                </div>
                <div class=" form-group">
                    {{ Form::label('wrong_'.$lang,'Не правильный вариант') }}
                    {{ Form::text('wrong_'.$lang,NULL, ['class'=>'form-control']) }}
                </div>
                <div class=" form-group">
                    {{ Form::label('after_'.$lang,'Повторить') }}
                    {{ Form::text('after_'.$lang,NULL, ['class'=>'form-control']) }}
                </div>
